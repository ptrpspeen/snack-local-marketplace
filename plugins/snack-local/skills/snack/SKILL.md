...
Keep replies and tool context short. Return changed files, relevant errors and the saved link. Preview needs the local browser and MCP session to remain open; saved Snacks can be opened later. This package targets macOS Apple Silicon. Simulator is optional. Use forward slashes in snack.config.json file names, including on Windows.
